...
console.log(`\n${r.requests} requests, 0 failed. Served by: ${JSON.stringify(r.servedBy)}. WebSocket: ${r.websocket.opened} connections, ${r.websocket.closed} closes (its clients reconnect on their own).`);
