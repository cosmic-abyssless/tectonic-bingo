...
# ---- the deploys under load: a probe (a stand-in for users) requests the site throughout, and must see no failures ----
probe_half() {
...
# CI runs the two halves as separate jobs, side by side (ZD_PART, above). Run with neither set, it does both, in order.
case "$part" in
  all) probe_half; guards_half ;;
  probe) probe_half ;;
  guards)
