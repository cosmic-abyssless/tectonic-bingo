...
| `stage.changed` | `bingoService.advanceStage` (in tx; keep the existing `stage_transitions` insert) | bingo / name | — | public | `{from, to}` (`startsAtBackfilled` only on entries from before a stage change stopped filling in the start date) |
