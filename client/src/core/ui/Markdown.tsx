...
const components: Components = {
  ...
  a: ({ href, ...p }) => {
    const className = "text-on-surface underline decoration-outline-strong underline-offset-2 hover:decoration-on-surface";
    // A link to another page of this site opens in the same tab; anything else opens in a new one.
    if (href?.startsWith("/")) return <Link to={href} className={className}>{p.children}</Link>;
    return <a className={className} target="_blank" rel="noreferrer" href={href} {...p} />;
  },
