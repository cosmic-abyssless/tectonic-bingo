...
function BugReportRow({ report, bingo }: { report: BugReportWithReporter; bingo: Bingo | null }) {
  ...
  return (
    <li className="space-y-1.5 px-4 py-3">
      ...
      {bingo ? (
        <Link to={`/b/${bingo.slug}/mod`} className="inline-block text-xs text-fg-muted hover:underline">
          {bingo.name}
        </Link>
      ) : (
        <span className="text-xs text-fg-subtle">Not tied to a bingo</span>
      )}
  ...
  return (
    <Card className="w-full max-w-2xl">
      ...
      {isLoading ? (
        ...
        <p className="px-5 pb-5 text-sm text-fg-subtle">No bug reports yet.</p>
      ) : (
