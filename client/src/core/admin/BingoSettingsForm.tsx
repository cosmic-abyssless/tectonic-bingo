...
import type { AchievementKey, Bingo, CutMode, ExclusivityRule, SignupMode } from "@bingo/shared";
...
export function BingoSettingsForm({
  ...
  const [form, setForm] = useState({
    ...
    achievementsEnabled: bingo.achievementsEnabled,
  });
  // Per-catalogue-entry switches the admin has touched this session, over the fetched state (CONTEXT.md
  // "Achievement") — only these are sent, so someone else's concurrent change to a key nobody touched here survives.
  const [achievementEdits, setAchievementEdits] = useState<Partial<Record<AchievementKey, boolean>>>({});
  const { data: achievementSettings } = useAchievementSettings(slug);
  ...
  async function save() {
    ...
    try {
      await adminApi.updateBingoSettings(slug, {
        ...
        ...(Object.keys(achievementEdits).length > 0 ? { achievements: achievementEdits } : {}),
      });
  ...
  return (
    <div className="max-w-2xl space-y-4">
      ...
      <Section title="Achievements">
        <p className="text-sm text-on-surface-muted">
          A just-for-fun layer for players — never touches points, scoring or the board (CONTEXT.md "Achievement"). Switching one off only hides it: earning continues in the background, and switching it
          back on restores everything players had.
        </p>
        <Switch isSelected={form.achievementsEnabled} onChange={(achievementsEnabled) => setForm({ ...form, achievementsEnabled })}>
          Achievements
        </Switch>
        <div className={`divide-y divide-outline rounded-md border border-outline ${form.achievementsEnabled ? "" : "opacity-50"}`}>
          {(achievementSettings?.achievements ?? []).map((a) => (
            <div key={a.key} className="flex items-center gap-3 px-3 py-2.5">
              ...
              <Switch
                isSelected={achievementEdits[a.key] ?? a.enabled}
                isDisabled={!form.achievementsEnabled}
                onChange={(enabled) => setAchievementEdits((prev) => ({ ...prev, [a.key]: enabled }))}
