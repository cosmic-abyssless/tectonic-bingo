...
export function QuestionBuilder({ slug }: { slug: string }) {
  ...
  async function add() {
    ...
    await adminApi.createQuestion(slug, { prompt: newPrompt.trim(), type: newType, sortOrder: questions.length });
    setNewPrompt("");
    invalidate();
  }
    ...
    <div className="max-w-2xl space-y-3">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="flex-1 bg-slate-900 border border-slate-600 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 placeholder:text-slate-600"
        />
        <select value={newType} onChange={(e) => setNewType(e.target.value as SignupQuestionType)} className="bg-slate-900 border border-slate-600 text-slate-300 text-sm rounded-md px-2 py-2 focus:outline-none">
          {TYPES.map((t) => (
            <option key={t.value} value={t.value}>
              {t.label}
            </option>
        ...
        <button onClick={add} className="text-sm bg-slate-700 hover:bg-slate-600 text-white rounded-md px-4 py-2 transition-colors cursor-pointer">
          Add
        </button>
      </div>
    </div>
