  ...
  return (
    <div className="p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-yellow-400 font-semibold text-sm">{side.points} pts</span>
      </div>

      <p className="text-slate-300 text-sm leading-relaxed mb-3">{side.description}</p>

      {required.length > 0 && (
        <ul className="space-y-1 mb-2">
          {required.map(item => (
            <li key={item.id} className="flex items-baseline gap-2 text-sm text-slate-200">
              <span className="text-indigo-400 text-xs">▸</span>
              {item.quantity > 1 && <span className="text-yellow-400 font-semibold text-xs">{item.quantity}×</span>}
              {item.itemName}
            </li>
          ))}
        </ul>
      ...
      {[...grouped.entries()].map(([group, opts]) => (
        <div key={group} className="mt-2">
          ...
          <ul className="space-y-1 mt-1">
            {opts.map(item => (
              <li key={item.id} className="flex items-baseline gap-2 text-sm text-slate-300">
      ...
      {(side.requiresNoDuplicates || side.allowsPreviouslyAcquired || side.allowsPreLoad) && (
        <div className="flex gap-2 flex-wrap mt-3">
          {side.requiresNoDuplicates     && <span className="text-xs bg-blue-900/40 text-blue-300 border border-blue-600 rounded-full px-2 py-0.5">No duplicates</span>}
          {side.allowsPreviouslyAcquired && <span className="text-xs bg-blue-900/40 text-blue-300 border border-blue-600 rounded-full px-2 py-0.5">Prev. acquired OK</span>}
          {side.allowsPreLoad            && <span className="text-xs bg-blue-900/40 text-blue-300 border border-blue-600 rounded-full px-2 py-0.5">Pre-load allowed</span>}
        </div>
      )}

      {side.notes && (
        <p className="mt-3 text-xs text-amber-400 border-l-2 border-amber-500 pl-2">{side.notes}</p>
      )}
    </div>
  );
}
...
export function TileModal({ tile, onClose }: Props) {
  const badgeColor = BADGE_COLORS[tile.badgeCategory] ?? "text-slate-400 border-slate-500 bg-slate-500/10";
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className={`flex items-start justify-between p-5 border-b-2 ${borderColor}`}>
          <div>
            <h2 className="text-white text-xl font-bold">{tile.name}</h2>
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              <span className={`text-xs font-semibold border rounded-full px-2.5 py-0.5 capitalize ${badgeColor}`}>
                {tile.badgeCategory.replace("_", " ")}
              </span>
              <span className="text-yellow-400 text-sm font-semibold">{tile.totalPoints} pts</span>
