...
export function ModPanel() {
  ...
  function setForm(id: string, patch: Partial<ReviewForm>) {
    setForms((prev) => ({
      ...prev,
      [id]: { ...getForm(submissions.find((s) => s.id === id)!), ...prev[id], ...patch },
    }));
  }
  ...
  return (
    <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col">
      ...
      <header className="flex items-center justify-between px-6 py-3 bg-slate-800 border-b border-slate-700 shrink-0">
        <span className="font-bold text-lg tracking-tight text-white">Mod Panel</span>
        <button
          onClick={() => navigate("/")}
          ...
          >
            ...
            {counts[key] > 0 && (
              <span className={`ml-1.5 text-xs rounded-full px-1.5 ${
                filter === key ? "bg-indigo-500 text-white" : "bg-slate-600 text-slate-300"
              }`}>
                {counts[key]}
              </span>
            )}
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    onClick={() => canReview && setExpandedId(isExpanded ? null : sub.id)}
                  >
                    {/* Thumbnail */}
                    {thumb ? (
                      <a
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className={`inline-block w-2 h-2 rounded-full shrink-0 ${BADGE_DOT[sub.badgeCategory] ?? "bg-slate-500"}`} />
                      ...
                      {sub.reviewerNotes && (
                        <p className="text-xs text-amber-400 mt-0.5 truncate">{sub.reviewerNotes}</p>
                      )}
                    </div>

                    {/* Status + time + expand hint */}
                    <div className="shrink-0 text-right flex flex-col items-end gap-1">
                      <span className={`text-xs font-semibold border rounded-full px-2 py-0.5 ${cls}`}>{label}</span>
                      {sub.status === "approved" && sub.pointsAwarded != null && (
                        <span className="text-xs text-yellow-400 font-semibold">{sub.pointsAwarded} pts</span>
                      )}
                  ...
                  {isExpanded && canReview && (
                    ...
                    >
                      ...
                      {sub.screenshots.length > 0 && (
                        <div className="space-y-2">
                          {sub.screenshots.map((ss, i) => (
                            <a key={i} href={ss.url} target="_blank" rel="noreferrer" title="Open full size in new tab">
                              <img
                                src={ss.url}
                                alt={ss.type}
                      ...
                      <div className="flex gap-3">
                        <div className="flex-1">
                          <label className="block text-xs font-medium text-slate-400 mb-1">
                            Points to award
                            <span className="text-slate-600 ml-1">(max {sub.sidePoints})</span>
                          </label>
                          <input
                            type="number"
                            min={0}
                            max={sub.sidePoints}
                            value={form.points}
                            onChange={(e) => setForm(sub.id, { points: e.target.value })}
                      ...
                      <div>
                        ...
                        <textarea
                          value={form.notes}
                          onChange={(e) => setForm(sub.id, { notes: e.target.value })}
