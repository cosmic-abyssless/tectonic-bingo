...
export function OnTheClockBanner({ teamName, teamColor, captains, pickLabel, isMyTurn, embedded, compact }: OnTheClockProps) {
  ...
  return (
    ...
    <motion.div
      ...
      role="status"
      aria-live="polite"
      className={`flex flex-wrap items-center justify-between gap-x-6 gap-y-1 ${compact ? "px-3 py-2" : "px-5 py-3"} ${embedded ? "" : "rounded-lg shadow-[0_4px_16px_var(--color-shade)]"}`}
